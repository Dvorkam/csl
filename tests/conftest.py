...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "macos_only: skip on non-macOS platforms")
...
def pytest_runtest_setup(item: pytest.Item) -> None:
    ...
    if item.get_closest_marker("macos_only") and platform.system() != "Darwin":
        pytest.skip("macos_only: skipped on non-macOS platform")
