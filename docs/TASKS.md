...
- [ ] **10.10** End-to-end smoke test from a clean NAS: run `setup.sh`, register one Linux target, one Windows target, run scripts on each. (Partial: `tests/e2e/test_docker_stack.py` now builds the prod image and smoke-tests the running app+nginx stack over the network in the `e2e` CI job. Still open: registering real Linux + Windows targets and running scripts. Note the `*Windows` e2e variants in `tests/e2e/` are `windows_only` but `e2e.yml` is `ubuntu-latest`-only, so they're skipped everywhere — a Windows-target e2e needs a Windows runner or a separate harness.)
