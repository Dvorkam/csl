...
- [x] **8.5.6** Cookie `secure` flag config-driven: new setting (`CSL_COOKIE_SECURE`, default `true`); both `web/auth.py` and `api/auth.py` honour it; dev `.env` sets `false` for plain-HTTP localhost. Tests keep using `base_url="https://testserver"`.
- [x] **8.5.7** Update ARCHITECTURE.md to match: §3.1/§6.2 (restricted key entry is standard, bundle carries the API token), §3.2 (job request carries `expected_md5`), §5.1 (new machine columns), §7.1 (bcrypt direct, passlib dropped — also fix the §9.1 pyproject excerpt), §7.3 (host-key pinning), §7.4/§7.6 (the "no general shell execution path" claim is now backed by the forced command; describe the gateway).

---

## Phase 9 — Audit log and observability

- [x] **9.1** Implement `server/api/audit.py` with admin-only read access. This is the JSON API counterpart of the admin web viewer from 8.12 — flesh out the empty router stub already wired into `main.py`, reusing the viewer's filter semantics (action, target_type, username, pagination).
...
- [x] **9.6** Stable error codes per §10: exceptions surfaced as structured error responses with a stable `code` field. Define a small initial catalogue (auth, approval, agent-unreachable, validation).
...
- [x] **10.7** Release version scheme decided: **tag-driven** (the pushed `v*` tag is the version; `pyproject.toml` holds only a dev placeholder that `release.yml` overwrites at build time — no auto-versioning). `release.yml` builds sdist+wheel and publishes via PyPI **Trusted Publishing (OIDC)** — pre-release tags → TestPyPI only, final `vX.Y.Z` → TestPyPI + PyPI. A `verify-install` job installs `control-station-lite[agent]` from TestPyPI on Linux **and** Windows and runs `csl-agent --help`. (Pipeline built; first real validation happens when a tag is pushed.)
- [x] **10.8** `release.yml` `docker` job builds the prod image (`deploy/Dockerfile` target `prod`) and pushes it to GHCR (`ghcr.io/dvorkam/control-station-lite`) tagged to match the release version, `latest` on final tags, authenticated with the built-in `GITHUB_TOKEN` (§9.2).
...
- [ ] **10.10** End-to-end smoke test from a clean NAS: run `setup.sh`, register one Linux target, one Windows target, run scripts on each. (Partial: `tests/e2e/test_docker_stack.py` builds the prod image and smoke-tests the running app+nginx stack over the network on the Linux `e2e` leg; `e2e.yml` is now an `ubuntu-latest` + `windows-latest` matrix, so the in-process `windows_only` agent flows (PowerShell staging→approve→run) finally execute on real Windows. Still open: a true clean-NAS run that registers real remote Linux + Windows *targets* over SSH and runs scripts end-to-end.)
