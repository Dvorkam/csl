...
- Private keys for each target are encrypted with AES-256-GCM. The key material is read directly from `secrets/master.key`, which contains exactly 32 random bytes stored as base64 (generated by `openssl rand -base64 32`). No KDF is applied — the file content is high-entropy by construction.
- A unique 12-byte nonce is generated per encryption operation and stored alongside the ciphertext (nonce || ciphertext format).
